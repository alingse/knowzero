...
5. 保持内容聚焦，不要过于冗长
6. **如果内容涉及系统架构、流程逻辑或复杂的组件关系，请使用 Mermaid 生成架构图。**
{context_instruction}
输出格式要求：
...
5. 包含与里程碑知识点相关的实践示例
6. 内容必须递进，不要与已有文档重复
7. **如果涉及技术架构或工作流程，请使用 Mermaid 生成对应的架构图或时序图。**
{context_instruction}
输出格式要求：
直接输出 Markdown 文档内容，不要包含额外的说明。"""
...
6. 保持内容聚焦，直接解决用户的问题
7. **对于复杂的概念解释，可以配合使用 Mermaid 图表（如架构图、流程图）来辅助理解。**
...
4. **如果用户提到需要架构图或逻辑流程不清晰，请补充 Mermaid 架构图。**"""
